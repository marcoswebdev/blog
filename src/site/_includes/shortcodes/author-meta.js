...
module.exports = eleventyConfig =>
  ...
  eleventyConfig.addShortcode('authorMeta', data => {
    if (data.author.name.fullName) {
      return `<meta name="author" content="${data.author.name.fullName}">`;
    }
    if (data.copyright.holder) {
      return `<meta name="author" content="${data.copyright.holder}">`;
    }
    return `<meta name="author" content="${data.site.title}">`;
  });
